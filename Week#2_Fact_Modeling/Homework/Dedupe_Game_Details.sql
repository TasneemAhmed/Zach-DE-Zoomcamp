...
-- Test1: Validate and test dim_game_date, dim_team_id, dim_player_id identify as unique row
select dim_game_date, dim_team_id, dim_player_id, count(1) as cnt
from fct_game_details
group by dim_game_date, dim_team_id, dim_player_id
having count(1)  >1;

-- Test2: analysis queries on fct_game_details table join with teams table
select fgd.*, t.*
from fct_game_details fgd
join teams t
on fgd.dim_team_id = t.team_id;

-- Test3: Calculate the percentage of players who did not play in any game
